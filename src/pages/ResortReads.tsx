        ...
        <Card className="print:shadow-none">
          ...
          <CardContent>
            <Tabs defaultValue="daily" className="w-full">
              ...
              <TabsContent value="daily" className="space-y-4 print:space-y-2 mt-6">
                <div className="flex justify-end mb-4 print:hidden">
                  <Button onClick={handleSaveAll}>Save All Daily Reads</Button>
                </div>
                {pools.map((pool) => (
                    ...
                    <CardContent className="py-2 print:py-1">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 print:gap-1">
                        <div className="space-y-1">
                          <Label htmlFor={`${pool.id}-chlorine`} className="text-xs print:text-[10px]">Chlorine</Label>
                          <Input
                            id={`${pool.id}-chlorine`}
                            type="number"
                            step="0.01"
                            value={reads[pool.id]?.chlorine || ""}
                            onChange={(e) => handleInputChange(pool.id, "chlorine", e.target.value)}
                            className="h-8 print:h-6 text-sm print:text-xs"
                          />
                        </div>
                        <div className="space-y-1">
                          <Label htmlFor={`${pool.id}-ph`} className="text-xs print:text-[10px]">pH</Label>
                          <Input
                            id={`${pool.id}-ph`}
                            type="number"
                            step="0.01"
                            value={reads[pool.id]?.ph || ""}
                            onChange={(e) => handleInputChange(pool.id, "ph", e.target.value)}
                            className="h-8 print:h-6 text-sm print:text-xs"
                          />
                        </div>
                        <div className="space-y-1">
                          <Label htmlFor={`${pool.id}-temperature`} className="text-xs print:text-[10px]">Temperature</Label>
                          <Input
                            id={`${pool.id}-temperature`}
                            type="number"
                            step="0.1"
                            value={reads[pool.id]?.temperature || ""}
                            onChange={(e) => handleInputChange(pool.id, "temperature", e.target.value)}
                            className="h-8 print:h-6 text-sm print:text-xs"
                          />
                        </div>
                        <div className="space-y-1">
                          <Label htmlFor={`${pool.id}-flow`} className="text-xs print:text-[10px]">Flow</Label>
                          <Input
                            id={`${pool.id}-flow`}
                            type="number"
                            step="0.01"
                            value={reads[pool.id]?.flow || ""}
                            onChange={(e) => handleInputChange(pool.id, "flow", e.target.value)}
                            className="h-8 print:h-6 text-sm print:text-xs"
                          />
                        </div>
                        {pool.pool_type === "standard" && (
                          <>
                            <div className="space-y-1">
                              <Label htmlFor={`${pool.id}-influent`} className="text-xs print:text-[10px]">Influent</Label>
                              <Input
                                id={`${pool.id}-influent`}
                                type="number"
                                step="0.01"
                                value={reads[pool.id]?.influent || ""}
                                onChange={(e) => handleInputChange(pool.id, "influent", e.target.value)}
                                className="h-8 print:h-6 text-sm print:text-xs"
                              />
                            </div>
                            <div className="space-y-1">
                              <Label htmlFor={`${pool.id}-effluent`} className="text-xs print:text-[10px]">Effluent</Label>
                              <Input
                                id={`${pool.id}-effluent`}
                                type="number"
                                step="0.01"
                                value={reads[pool.id]?.effluent || ""}
                                onChange={(e) => handleInputChange(pool.id, "effluent", e.target.value)}
                                className="h-8 print:h-6 text-sm print:text-xs"
                              />
                            </div>
                          </>
                  ...
                  <Card key={pool.id} className="print:break-inside-avoid print:shadow-none">
                    ...
                    <CardContent className="print:py-2">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 print:gap-2">
                        <div className="space-y-1">
                          <Label htmlFor={`${pool.id}-tds`} className="text-xs">TDS</Label>
                          <Input
                            id={`${pool.id}-tds`}
                            type="number"
                            step="0.01"
                            value={weeklyReads[pool.id]?.tds || ""}
                            onChange={(e) => handleWeeklyInputChange(pool.id, "tds", e.target.value)}
                            className="h-9 print:h-8 text-sm"
                          />
                        </div>
                        <div className="space-y-1">
                          <Label htmlFor={`${pool.id}-alkalinity`} className="text-xs">Alkalinity</Label>
                          <Input
                            id={`${pool.id}-alkalinity`}
                            type="number"
                            step="0.01"
                            value={weeklyReads[pool.id]?.alkalinity || ""}
                            onChange={(e) => handleWeeklyInputChange(pool.id, "alkalinity", e.target.value)}
                            className="h-9 print:h-8 text-sm"
                          />
                        </div>
                        <div className="space-y-1">
                          <Label htmlFor={`${pool.id}-calcium`} className="text-xs">Calcium Hardness</Label>
                          <Input
                            id={`${pool.id}-calcium`}
                            type="number"
                            step="0.01"
                            value={weeklyReads[pool.id]?.calciumHardness || ""}
                            onChange={(e) => handleWeeklyInputChange(pool.id, "calciumHardness", e.target.value)}
                            className="h-9 print:h-8 text-sm"
                          />
                        </div>
                        <div className="space-y-1">
                          <Label htmlFor={`${pool.id}-lsi`} className="text-xs">Saturation Index (LSI)</Label>
                          <Input
                            id={`${pool.id}-lsi`}
                            type="number"
                            step="0.01"
                            value={weeklyReads[pool.id]?.lsi !== null ? weeklyReads[pool.id].lsi : ""}
                            readOnly
                            placeholder="Auto-calculated"
                            className="h-9 print:h-8 text-sm bg-muted"
                          />
                          {weeklyReads[pool.id]?.lsi !== null && (
                            <p className="text-xs mt-1">
                              {weeklyReads[pool.id].lsi! < -0.5 ? (
                                <span className="text-destructive font-medium">⚠️ Corrosive - Needs Treatment</span>
                              ) : weeklyReads[pool.id].lsi! > 0.5 ? (
                                <span className="text-destructive font-medium">⚠️ Scale-Forming - Needs Treatment</span>
                              ) : (
                                <span className="text-green-600 font-medium">✓ Balanced - No Treatment Needed</span>
                              )}
                            </p>
                          )}
                        </div>
