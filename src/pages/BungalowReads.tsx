...
const BungalowReads = () => {
  ...
  const fetchBungalows = async () => {
    ...
    if (error) {
      ...
      // Sort numerically by extracting the number from the name
              ...
              <CardContent className="py-2 print:py-1">
                <div className="grid grid-cols-4 gap-2 print:gap-1">
                  <div className="space-y-1">
                    <Label htmlFor={`${bungalow.id}-chlorine`} className="text-xs">Chlorine</Label>
                    <Input
                      id={`${bungalow.id}-chlorine`}
                      type="number"
                      step="0.01"
                      value={reads[bungalow.id]?.chlorine || ""}
                      onChange={(e) => handleInputChange(bungalow.id, "chlorine", e.target.value)}
                      className="h-8 text-sm print:h-6 print:text-xs"
                    />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor={`${bungalow.id}-ph`} className="text-xs">pH</Label>
                    <Input
                      id={`${bungalow.id}-ph`}
                      type="number"
                      step="0.01"
                      value={reads[bungalow.id]?.ph || ""}
                      onChange={(e) => handleInputChange(bungalow.id, "ph", e.target.value)}
                      className="h-8 text-sm print:h-6 print:text-xs"
                    />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor={`${bungalow.id}-temperature`} className="text-xs">Temperature</Label>
                    <Input
                      id={`${bungalow.id}-temperature`}
                      type="number"
                      step="0.1"
                      value={reads[bungalow.id]?.temperature || ""}
                      onChange={(e) => handleInputChange(bungalow.id, "temperature", e.target.value)}
                      className="h-8 text-sm print:h-6 print:text-xs"
                    />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor={`${bungalow.id}-flow`} className="text-xs">Flow</Label>
                    <Input
                      id={`${bungalow.id}-flow`}
                      type="number"
                      step="0.01"
                      value={reads[bungalow.id]?.flow || ""}
                      onChange={(e) => handleInputChange(bungalow.id, "flow", e.target.value)}
                      className="h-8 text-sm print:h-6 print:text-xs"
                    />
                  </div>
                </div>
          ...
          {group.map((bungalow) => (
            <Card key={bungalow.id} className="print:break-inside-avoid print:shadow-none">
              ...
              <CardContent className="py-2 print:py-1">
                <div className="grid grid-cols-4 gap-2 print:gap-1">
                  <div className="space-y-1">
                    <Label htmlFor={`${bungalow.id}-tds`} className="text-xs">TDS</Label>
                    <Input
                      id={`${bungalow.id}-tds`}
                      type="number"
                      step="0.01"
                      value={weeklyReads[bungalow.id]?.tds || ""}
                      onChange={(e) => handleWeeklyInputChange(bungalow.id, "tds", e.target.value)}
                      className="h-8 text-sm print:h-6 print:text-xs"
                    />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor={`${bungalow.id}-alkalinity`} className="text-xs">Alkalinity</Label>
                    <Input
                      id={`${bungalow.id}-alkalinity`}
                      type="number"
                      step="0.01"
                      value={weeklyReads[bungalow.id]?.alkalinity || ""}
                      onChange={(e) => handleWeeklyInputChange(bungalow.id, "alkalinity", e.target.value)}
                      className="h-8 text-sm print:h-6 print:text-xs"
                    />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor={`${bungalow.id}-calcium`} className="text-xs">Calcium Hardness</Label>
                    <Input
                      id={`${bungalow.id}-calcium`}
                      type="number"
                      step="0.01"
                      value={weeklyReads[bungalow.id]?.calciumHardness || ""}
                      onChange={(e) => handleWeeklyInputChange(bungalow.id, "calciumHardness", e.target.value)}
                      className="h-8 text-sm print:h-6 print:text-xs"
                    />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor={`${bungalow.id}-lsi`} className="text-xs">LSI</Label>
                    <Input
                      id={`${bungalow.id}-lsi`}
                      type="number"
                      value={weeklyReads[bungalow.id]?.lsi !== null ? weeklyReads[bungalow.id].lsi : ""}
                      readOnly
                      placeholder="Auto"
                      className="h-8 text-sm print:h-6 print:text-xs bg-muted"
                    />
                    {weeklyReads[bungalow.id]?.lsi !== null && (
                      <p className="text-xs mt-1">
                        {weeklyReads[bungalow.id].lsi! < -0.5 ? (
                          <span className="text-destructive font-medium">⚠️ Corrosive</span>
                        ) : weeklyReads[bungalow.id].lsi! > 0.5 ? (
                          <span className="text-destructive font-medium">⚠️ Scale-Forming</span>
                        ) : (
    ...
    <div className="min-h-screen bg-background p-4">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <Card className="print:shadow-none">
          <CardHeader>
            <CardTitle className="text-2xl">Polynesian Bungalows - All Units</CardTitle>
          </CardHeader>
          <CardContent>
            <Tabs defaultValue="daily" className="w-full">
              ...
              <TabsContent value="daily" className="mt-6">
                <div className="flex justify-end mb-4 print:hidden">
                  <Button onClick={handleSave}>Save All Daily Reads</Button>
                </div>
                <div>
                  {renderBungalowGroup(1, 10, "Polynesian Bungalows 1-10")}
                  {renderBungalowGroup(11, 20, "Polynesian Bungalows 11-20")}
                </div>
              </TabsContent>
              ...
              <TabsContent value="weekly" className="mt-6">
                ...
                <div>
                  {renderWeeklyBungalowGroup(1, 10, "Polynesian Bungalows 1-10")}
                  {renderWeeklyBungalowGroup(11, 20, "Polynesian Bungalows 11-20")}
                </div>
              </TabsContent>
            </Tabs>
          </CardContent>
